...
export function DetailedStatsTable({ data }: DetailedStatsTableProps) {
    ...
    const statsLeft = [
        { label: 'Total P&L', value: formatCurrency(data.totalPnL), color: parseFloat(data.totalPnL) >= 0 ? 'text-blue-400' : 'text-red-400' },
    ...
    const statsRight = [
        ...
        { label: 'Average daily P&L', value: formatCurrency(extended.averageDailyPnL), color: extended.averageDailyPnL >= 0 ? 'text-blue-400' : 'text-red-400' },
        ...
        { label: 'Trade expectancy', value: formatCurrency(data.expectancy), color: parseFloat(data.expectancy) >= 0 ? 'text-blue-400' : 'text-red-400' },
        { label: 'Max drawdown', value: formatCurrency(-data.maxDrawdown), color: 'text-red-400' },
