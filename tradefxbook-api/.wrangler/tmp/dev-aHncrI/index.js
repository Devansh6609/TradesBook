...
// .wrangler/tmp/bundle-Htam72/checked-fetch.js
...
// .wrangler/tmp/bundle-Htam72/strip-cf-connecting-ip-header.js
...
async function queryMany(db, sql, params = []) {
...
__name(toUnix, "toUnix");
function fromUnix(ts) {
  if (ts == null)
    return null;
  return new Date(ts * 1e3).toISOString();
}
__name(fromUnix, "fromUnix");

// src/lib/auth.ts
var ACCESS_TOKEN_TTL = 60 * 15;
var REFRESH_TOKEN_TTL = 60 * 60 * 24 * 30;
function getSecret(jwtSecret) {
  return new TextEncoder().encode(jwtSecret);
}
__name(getSecret, "getSecret");
async function signAccessToken(payload, jwtSecret) {
  return new SignJWT({ userId: payload.userId, email: payload.email }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime(`15m`).sign(getSecret(jwtSecret));
}
__name(signAccessToken, "signAccessToken");
async function verifyAccessToken(token, jwtSecret) {
...
async function hashPassword(password) {
  ...
  const toHex = /* @__PURE__ */ __name((arr) => Array.from(arr, (b) => b.toString(16).padStart(2, "0")).join(""), "toHex");
...
async function verifyPassword(password, stored) {
  ...
  if (!saltHex || !hashHex)
    return false;
  const fromHex = /* @__PURE__ */ __name((hex) => new Uint8Array(hex.match(/.{2}/g).map((b) => parseInt(b, 16))), "fromHex");
...
function sessionExpiresAt() {
  return toUnix(new Date(Date.now() + REFRESH_TOKEN_TTL * 1e3));
}
...
auth.post("/login", async (c) => {
  ...
  const user = await queryOne(c.env.DB, "SELECT id, email, name, image, password FROM users WHERE email = ?", [email]);
...
function calculatePnL(type, entryPrice, exitPrice, quantity, symbol, commission = 0, swap = 0, fees = 0, stopLoss) {
...
__name(calculatePnL, "calculatePnL");
function serializeTrade(row) {
  return {
    ...
    entryDate: fromUnix(row.entryDate),
    exitDate: fromUnix(row.exitDate),
...
__name(serializeTrade, "serializeTrade");
var emptyToUndefined = external_exports.string().optional().transform((v) => v === "" ? void 0 : v);
var querySchema = external_exports.object({
  page: external_exports.string().optional().default("1"),
  limit: external_exports.string().optional().default("25"),
  symbol: emptyToUndefined,
  status: emptyToUndefined.pipe(external_exports.enum(["OPEN", "CLOSED", "CANCELLED", "PENDING"]).optional()),
  type: emptyToUndefined.pipe(external_exports.enum(["BUY", "SELL"]).optional()),
  ...
  sortBy: emptyToUndefined.pipe(external_exports.enum(["entryDate", "exitDate", "pnl", "symbol", "createdAt"]).optional()).transform((v) => v ?? "entryDate"),
  sortOrder: emptyToUndefined.pipe(external_exports.enum(["asc", "desc"]).optional()).transform((v) => v ?? "desc")
});
trades.get("/", async (c) => {
  const userId = c.get("userId");
  const raw2 = Object.fromEntries(new URL(c.req.url).searchParams);
  const parsed = querySchema.safeParse(raw2);
  if (!parsed.success)
    return c.json({ error: "Invalid query params", details: parsed.error.errors }, 400);
  const p = parsed.data;
  const page = parseInt(p.page);
  const limit = Math.min(parseInt(p.limit), 100);
  const offset = (page - 1) * limit;
  const conditions = ["t.userId = ?"];
  const bindings = [userId];
  if (p.symbol) {
    conditions.push("t.symbol LIKE ?");
    bindings.push(`%${p.symbol.toUpperCase()}%`);
  }
  if (p.status) {
    conditions.push("t.status = ?");
    bindings.push(p.status);
  }
  if (p.type) {
    conditions.push("t.type = ?");
    bindings.push(p.type);
  }
  if (p.strategyId) {
    conditions.push("t.strategyId = ?");
    bindings.push(p.strategyId);
  }
  if (p.dateFrom) {
    conditions.push("t.entryDate >= ?");
    bindings.push(toUnix(p.dateFrom));
  }
  if (p.dateTo) {
    conditions.push("t.entryDate <= ?");
    bindings.push(toUnix(p.dateTo));
  }
  if (p.exitDateFrom) {
    conditions.push("t.exitDate >= ?");
    bindings.push(toUnix(p.exitDateFrom));
  }
  if (p.exitDateTo) {
    conditions.push("t.exitDate <= ?");
    bindings.push(toUnix(p.exitDateTo));
  }
  if (p.minPnl !== void 0) {
    conditions.push("t.pnl >= ?");
    bindings.push(parseFloat(p.minPnl));
  }
  if (p.maxPnl !== void 0) {
    conditions.push("t.pnl <= ?");
    bindings.push(parseFloat(p.maxPnl));
  }
  ...
  return c.json({
    trades: rows.map((row) => ({
      ...serializeTrade(row),
      strategy: row.strategyId ? { id: row.strategyId, name: row.strategy_name } : null,
      tags: tagsMap[row.id] ?? [],
      screenshots: screenshotsMap[row.id] ?? []
    })),
...
var createSchema = external_exports.object({
  symbol: external_exports.string().min(1).transform((s) => s.toUpperCase()),
  type: external_exports.enum(["BUY", "SELL"]),
  entryPrice: external_exports.number().positive(),
  exitPrice: external_exports.number().optional(),
  entryDate: external_exports.string(),
  exitDate: external_exports.string().optional(),
  quantity: external_exports.number().positive(),
  stopLoss: external_exports.number().optional(),
  takeProfit: external_exports.number().optional(),
  commission: external_exports.number().default(0),
  swap: external_exports.number().default(0),
  fees: external_exports.number().default(0),
  status: external_exports.enum(["OPEN", "CLOSED", "CANCELLED", "PENDING"]).default("OPEN"),
  strategyId: external_exports.string().optional(),
  setupType: external_exports.string().optional(),
  marketCondition: external_exports.enum(["TRENDING_UP", "TRENDING_DOWN", "RANGING", "VOLATILE", "BREAKOUT", "REVERSAL"]).optional(),
  entryEmotion: external_exports.enum(["CONFIDENT", "FEARFUL", "GREEDY", "IMPATIENT", "NEUTRAL", "FRUSTRATED", "EXCITED", "CAUTIOUS"]).optional(),
  exitEmotion: external_exports.enum(["CONFIDENT", "FEARFUL", "GREEDY", "IMPATIENT", "NEUTRAL", "FRUSTRATED", "EXCITED", "CAUTIOUS"]).optional(),
  preTradeAnalysis: external_exports.string().optional(),
  postTradeAnalysis: external_exports.string().optional(),
  ...
  partialCloses: external_exports.array(external_exports.object({
    quantity: external_exports.number().positive(),
    exitPrice: external_exports.number().positive(),
...
trades.post("/", async (c) => {
  ...
  if (!entryDate)
    return c.json({ error: "Invalid entry date" }, 400);
  let pnl = null, pnlPct = null;
  let netPnl = null, rMultiple = null;
  ...
  await execute(
    ...
    [
      ...
      d.strategyId ?? null,
      d.setupType ?? null,
      d.marketCondition ?? null,
      d.entryEmotion ?? null,
      d.exitEmotion ?? null,
      d.preTradeAnalysis ?? null,
      d.postTradeAnalysis ?? null,
  ...
  const trade = await queryOne(c.env.DB, "SELECT * FROM trades WHERE id = ?", [id]);
  return c.json(serializeTrade(trade), 201);
});
var trades_default = trades;

// src/routes/tradeById.ts
var tradeById = new Hono2();
tradeById.use("*", authMiddleware);
function serializeTrade2(row) {
  return {
    id: row.id,
    userId: row.userId,
    accountId: row.accountId,
...
__name(serializeTrade2, "serializeTrade");
tradeById.get("/:id", async (c) => {
  const userId = c.get("userId");
  const { id } = c.req.param();
  const trade = await queryOne(
    c.env.DB,
    "SELECT * FROM trades WHERE id = ? AND userId = ?",
    [id, userId]
  );
  if (!trade)
    return c.json({ error: "Trade not found" }, 404);
  const [tags2, screenshots2, partialCloses] = await Promise.all([
    queryMany(
      c.env.DB,
    ...
    queryMany(
      c.env.DB,
      "SELECT id, url, caption, chartType FROM screenshots WHERE tradeId = ?",
      [id]
    ),
    queryMany(
      c.env.DB,
      "SELECT id, quantity, exitPrice, pnl, closedAt, notes FROM partial_closes WHERE tradeId = ? ORDER BY closedAt DESC",
      [id]
    )
  ]);
  return c.json({
    ...serializeTrade2(trade),
    tags: tags2.map((t) => ({ id: t.tagId, name: t.name, color: t.color })),
    screenshots: screenshots2.map((s) => ({ id: s.id, key: s.url, url: s.url, caption: s.caption, chartType: s.chartType })),
...
var updateSchema = external_exports.object({
  ...
  stopLoss: external_exports.number().optional(),
  takeProfit: external_exports.number().optional(),
  commission: external_exports.number().optional(),
  swap: external_exports.number().optional(),
  fees: external_exports.number().optional(),
  status: external_exports.enum(["OPEN", "CLOSED", "CANCELLED", "PENDING"]).optional(),
  strategyId: external_exports.string().nullable().optional(),
  setupType: external_exports.string().optional(),
  marketCondition: external_exports.string().optional(),
  entryEmotion: external_exports.string().optional(),
  exitEmotion: external_exports.string().optional(),
  preTradeAnalysis: external_exports.string().optional(),
  postTradeAnalysis: external_exports.string().optional(),
  lessonsLearned: external_exports.string().optional(),
  rating: external_exports.number().int().min(1).max(5).optional(),
...
tradeById.put("/:id", async (c) => {
  ...
  for (const [key, col] of Object.entries(map)) {
    if (key in d && d[key] !== void 0) {
      let val = d[key];
      if (key === "exitDate" && typeof val === "string")
        val = toUnix(val);
      fields.push(`${col} = ?`);
      vals.push(val);
    }
  ...
  const updated = await queryOne(c.env.DB, "SELECT * FROM trades WHERE id = ?", [id]);
  return c.json(serializeTrade2(updated));
});
...
var tradeById_default = tradeById;

// src/routes/screenshots.ts
var screenshots = new Hono2();
...
var screenshots_default = screenshots;

// src/routes/strategies.ts
var strategies = new Hono2();
strategies.use("*", authMiddleware);
var schema = external_exports.object({
  name: external_exports.string().min(1),
  description: external_exports.string().optional(),
  rules: external_exports.string().optional(),
...
strategies.post("/", async (c) => {
  ...
  const row = await queryOne(c.env.DB, "SELECT * FROM strategies WHERE id = ?", [id]);
...
strategies.put("/:id", async (c) => {
  ...
  if (d.description !== void 0) {
    ...
    vals.push(d.description);
  }
  if (d.rules !== void 0) {
    fields.push("rules = ?");
    vals.push(d.rules);
  }
  if (d.isActive !== void 0) {
...
var schema2 = external_exports.object({
  name: external_exports.string().min(1),
  color: external_exports.string().regex(/^#[0-9a-fA-F]{6}$/).optional().default("#3b82f6"),
  description: external_exports.string().optional()
});
...
tags.get("/", async (c) => {
  ...
  return c.json({ tags: rows.map(serialize2) });
});
tags.post("/", async (c) => {
  const userId = c.get("userId");
  let body;
  ...
  const row = await queryOne(c.env.DB, "SELECT * FROM tags WHERE id = ?", [id]);
  return c.json(serialize2(row), 201);
});
tags.put("/:id", async (c) => {
  const userId = c.get("userId");
  const { id } = c.req.param();
  ...
  const parsed = schema2.partial().safeParse(body);
  if (!parsed.success)
    return c.json({ error: "Validation failed", details: parsed.error.errors }, 400);
  const d = parsed.data;
  const now = Math.floor(Date.now() / 1e3);
...
function serialize3(row) {
  return {
    ...
    defaultTradeView: row.defaultTradeView,
    ...
    favoriteSymbols: row.favoriteSymbols?.split(",").filter(Boolean) ?? [],
...
settings.get("/", async (c) => {
  ...
  if (!row) {
    ...
    row = await queryOne(c.env.DB, "SELECT * FROM settings WHERE userId = ?", [userId]);
  }
  return c.json(serialize3(row));
});
var updateSchema2 = external_exports.object({
  accountBalance: external_exports.number().optional(),
  unrealizedPnl: external_exports.number().optional(),
  theme: external_exports.enum(["dark", "light", "system"]).optional(),
  currency: external_exports.string().optional(),
  dateFormat: external_exports.string().optional(),
  timezone: external_exports.string().optional(),
  defaultTradeView: external_exports.enum(["list", "grid", "calendar"]).optional(),
  tradesPerPage: external_exports.number().int().min(5).max(100).optional(),
  showClosedTrades: external_exports.boolean().optional(),
  showOpenTrades: external_exports.boolean().optional(),
  defaultDashboardView: external_exports.string().optional(),
  favoriteSymbols: external_exports.array(external_exports.string()).optional(),
  emailNotifications: external_exports.boolean().optional(),
  tradeAlerts: external_exports.boolean().optional(),
  weeklyReports: external_exports.boolean().optional(),
  publicProfile: external_exports.boolean().optional(),
  shareAnalytics: external_exports.boolean().optional(),
  showOnLeaderboard: external_exports.boolean().optional(),
  showTrades: external_exports.boolean().optional(),
  showPnlPerTrade: external_exports.boolean().optional(),
  showTotalPnl: external_exports.boolean().optional(),
  showWinRate: external_exports.boolean().optional(),
  showTradeCount: external_exports.boolean().optional(),
...
settings.put("/", async (c) => {
  ...
  const parsed = updateSchema2.safeParse(body);
  if (!parsed.success)
    return c.json({ error: "Validation failed", details: parsed.error.errors }, 400);
  const d = parsed.data;
  const now = Math.floor(Date.now() / 1e3);
  const colMap = {
    ...
    timezone: "timezone",
    defaultTradeView: "defaultTradeView",
    tradesPerPage: "tradesPerPage",
    showClosedTrades: "showClosedTrades",
    showOpenTrades: "showOpenTrades",
    ...
    showPnlPerTrade: "showPnlPerTrade",
  ...
  const row = await queryOne(c.env.DB, "SELECT * FROM settings WHERE userId = ?", [userId]);
  return c.json(serialize3(row));
});
...
analytics.get("/live", async (c) => {
  const userId = c.get("userId");
  try {
    const settings2 = await queryOne(
      c.env.DB,
    ...
    `).bind(userId).run();
    const activeSync = await queryOne(
      c.env.DB,
      `SELECT id FROM sync_requests WHERE userId = ? AND status IN ('PENDING', 'PROCESSING') ORDER BY createdAt DESC LIMIT 1`,
      [userId]
    );
    const initialBalance = settings2?.accountBalance ?? 0;
    const unrealizedPnl = settings2?.unrealizedPnl ?? 0;
  ...
  try {
    ...
    return c.json({
      overview: {
        ...
        winRate: (overview?.closed ?? 0) > 0 ? (overview?.winning ?? 0) / (overview?.closed ?? 1) * 100 : 0,
...
analytics.get("/", async (c) => {
  ...
  try {
    ...
    const initialBalance = settings2?.accountBalance ?? 0;
    const unrealizedPnlTotal = settings2?.unrealizedPnl ?? 0;
    let dateFilter = "";
    const params = [userId];
...
// src/routes/mt5.ts
var app = new Hono2();
var safeUnixTimestamp = /* @__PURE__ */ __name((dateStr) => {
  if (!dateStr)
    return null;
...
app.post("/sync", authMiddleware, async (c) => {
  const userId = c.get("userId");
  const body = await c.req.json();
  const { type, value } = external_exports.object({
    type: external_exports.enum(["COUNT", "DATE", "ALL"]),
...
var AccountUpdateSchema = external_exports.object({
  email: external_exports.string().email(),
  type: external_exports.literal("account_update"),
  balance: external_exports.number(),
  equity: external_exports.number(),
  freeMargin: external_exports.number()
});
var TradeUpdateSchema = external_exports.object({
  email: external_exports.string().email(),
  type: external_exports.literal("trade_update"),
  ticket: external_exports.string(),
  positionId: external_exports.string(),
  symbol: external_exports.string(),
  tradeType: external_exports.enum(["BUY", "SELL"]),
  ...
  pnl: external_exports.number(),
  commission: external_exports.number().default(0),
  swap: external_exports.number().default(0),
  status: external_exports.enum(["OPEN", "CLOSED", "CANCELLED", "PENDING"])
});
var WebhookPayloadSchema = external_exports.union([AccountUpdateSchema, TradeUpdateSchema]);
app.post("/", async (c) => {
  try {
    ...
    const user = await c.env.DB.prepare("SELECT id FROM users WHERE email = ?").bind(payload.email).first();
    if (!user) {
      return c.json({ error: "User not found for provided email" }, 404);
    }
    const userId = user.id;
    let account = await c.env.DB.prepare("SELECT id FROM accounts WHERE userId = ? LIMIT 1").bind(userId).first();
    let accountId = account?.id;
    if (!accountId) {
      accountId = generateId();
      await c.env.DB.prepare(
        "INSERT INTO accounts (id, userId, type, provider, providerAccountId, brokerName, isActive) VALUES (?, ?, ?, ?, ?, ?, ?)"
      ).bind(accountId, userId, "credentials", "mt5", `mt5-${userId}`, "MetaTrader 5", 1).run();
    }
    if (payload.type === "account_update") {
      const unrealizedPnl = payload.equity - payload.balance;
      await c.env.DB.prepare("UPDATE accounts SET accountBalance = ?, unrealizedPnl = ?, lastSyncAt = unixepoch() WHERE id = ?").bind(payload.balance, unrealizedPnl, accountId).run();
      await c.env.DB.prepare("UPDATE settings SET accountBalance = ?, unrealizedPnl = ?, updatedAt = unixepoch() WHERE userId = ?").bind(payload.balance, unrealizedPnl, userId).run();
      ...
      const existingTrade = await c.env.DB.prepare("SELECT id FROM trades WHERE ticketId = ?").bind(payload.ticket).first();
      if (existingTrade) {
        await c.env.DB.prepare(`
          UPDATE trades 
          ...
          WHERE ticketId = ?
        `).bind(
          payload.exitPrice !== void 0 ? payload.exitPrice : null,
          exitTime,
          payload.pnl,
          payload.commission,
          payload.swap,
    ...
    `).bind(userId).first();
    if (syncRequest) {
      await c.env.DB.prepare(`UPDATE sync_requests SET status = 'COMPLETED', updatedAt = unixepoch() WHERE id = ?`).bind(syncRequest.id).run();
      return c.json({
        success: true,
...
app2.get("/", (c) => c.json({ status: "ok", service: "TradesBook API", version: "2.0.0" }));
app2.get("/health", (c) => c.json({ status: "ok" }));
app2.route("/api/auth", auth_default);
app2.route("/api/trades", trades_default);
app2.route("/api/trades", tradeById_default);
app2.route("/api/trades", screenshots_default);
app2.route("/api/strategies", strategies_default);
app2.route("/api/tags", tags_default);
app2.route("/api/settings", settings_default);
app2.route("/api/analytics", analytics_default);
app2.route("/api/mt5-webhook", mt5_default);
app2.notFound((c) => c.json({ error: "Route not found" }, 404));
app2.onError((err, c) => {
...
// .wrangler/tmp/bundle-Htam72/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Htam72/middleware-loader.entry.ts
